...
export const listarProgramaciones = async (req, res) => {
  try {
    ...
    if (estado) {
      query += ` AND ps.estado = $${index}`;
      values.push(estado.toUpperCase());
      index++;
    }

    if (frecuencia) {
      query += ` AND ps.frecuencia = $${index}`;
      values.push(frecuencia.toUpperCase());
      index++;
    }

    if (prioridad) {
      query += ` AND ps.prioridad = $${index}`;
      values.push(prioridad.toUpperCase());
      index++;
    }

    if (id_cliente) {
      query += ` AND ps.id_cliente = $${index}`;
      values.push(id_cliente);
      index++;
    }

    if (id_propiedad) {
      query += ` AND ps.id_propiedad = $${index}`;
      values.push(id_propiedad);
      index++;
    }

    if (id_servicio) {
      query += ` AND ps.id_servicio = $${index}`;
      values.push(id_servicio);
      index++;
    }

    if (fecha_desde) {
      query += ` AND ps.proxima_fecha >= $${index}`;
    ...
    query += ` ORDER BY ps.proxima_fecha ASC, ps.id_programacion DESC`;
